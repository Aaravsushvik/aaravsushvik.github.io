document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateMetaColor=(isDark)=>{if(metaThemeColor)metaThemeColor.setAttribute('content',isDark?'#1c1c1e':'#fbfbfd');};
    const updateThemeIcon=()=>{
        ...
        if(isDark){moonIcon.classList.add('hidden');sunIcon.classList.remove('hidden');themeToggle.setAttribute('aria-label','Switch to light mode');}
        else{moonIcon.classList.remove('hidden');sunIcon.classList.add('hidden');themeToggle.setAttribute('aria-label','Switch to dark mode');}
    };
    ...
    try {
        ...
        if(savedTheme==='light'||savedTheme==='dark'){html.classList.remove('light','dark');html.classList.add(savedTheme);}
    } catch(e) {}
    ...
    const dispatchThemeUpdate=()=>{
        ...
        d.dispatchEvent(new CustomEvent('themechange',{detail:{
    ...
    systemTheme.addEventListener('change',(e)=>{
        try {
            if(!localStorage.getItem('theme')){
                html.classList.remove('light','dark');html.classList.add(e.matches?'dark':'light');
                updateThemeIcon();updateMetaColor(e.matches);requestAnimationFrame(dispatchThemeUpdate);
            }
        } catch(err) {}
    });
    ...
    if(themeToggle){
        themeToggle.addEventListener('click',()=>{
            ...
            html.classList.remove(currentlyDark?'dark':'light');html.classList.add(currentlyDark?'light':'dark');
            try{localStorage.setItem('theme',currentlyDark?'light':'dark');}catch(err){}
            updateMetaColor(!currentlyDark);updateThemeIcon();requestAnimationFrame(dispatchThemeUpdate);
        });
    }

    const canvas=q('hero-canvas'),heroSection=q('home');
    const QUALITY=Object.freeze({LOW:0,MEDIUM:1,HIGH:2});
    const CONFIG=Object.freeze({mouseRadius:150,mouseRadiusSq:22500,physicsStep:1000/60,alphaBuckets:5,levels:{0:{count:25,connDistSq:6400},1:{count:50,connDistSq:10000},2:{count:80,connDistSq:14400}}});
    ...
    if(canvas&&heroSection&&!prefersReducedMotion.matches&&!saveData){
        ...
        class Particle{
            ...
            reset(){this.x=Math.random()*width;this.y=Math.random()*height;this.vx=(Math.random()-0.5)*0.5;this.vy=(Math.random()-0.5)*0.5;this.radius=Math.random()*1.5+0.5;}
            update(dt){
                const timeScale=dt/CONFIG.physicsStep;this.x+=this.vx*timeScale;this.y+=this.vy*timeScale;
                ...
                if(mouse.x!==null){
                    ...
                    if(distSq>0&&distSq<CONFIG.mouseRadiusSq){
                        ...
                        this.x-=(dx/distance)*force*1.5*timeScale;this.y-=(dy/distance)*force*1.5*timeScale;
                    }
                }
            }
        ...
        const startEngine=()=>{if(engineRunning)return;engineRunning=true;lastTime=performance.now();accumulator=0;animationFrameId=requestAnimationFrame(animateCanvas);};
        ...
        const animateCanvas=(time)=>{
            ...
            if(rawPointer.x!==null){mouse.x=rawPointer.x;mouse.y=rawPointer.y;}else{mouse.x=null;mouse.y=null;}
        ...
        // Pause the engine as soon as the hero leaves the viewport.
        new IntersectionObserver(([entry])=>{
            heroVisible=entry.isIntersecting;
            if(heroVisible&&!d.hidden&&!prefersReducedMotion.matches)startEngine();else stopEngine();
        ...
        const watchDPR=()=>{
            w.matchMedia(`(resolution: ${w.devicePixelRatio}dppx)`)
             .addEventListener('change',()=>{initCanvas();watchDPR();},{once:true});
        };
        watchDPR();
    ...
    if(menuToggle&&nav){
        menuToggle.addEventListener('click',()=>{
            const isOpen=nav.classList.toggle('open');menuToggle.setAttribute('aria-expanded',String(isOpen));
            if(isOpen){
        ...
        // A viewport that crosses into desktop while the menu is open must release inert,
        // otherwise the hamburger disappears and the whole page stays uninteractive.
    ...
    const initDeferredObservers=()=>{
        if(navLinks.length>0&&sections.length>0){
            ...
            // Observe only sections that actually have a nav link, so the highlight
            // never blanks out on Skills / Milestones / Favorites / Inspiration.
            [...navLinks]
        ...
        }else{d.querySelectorAll('.reveal').forEach(el=>el.classList.add('visible'));}
    };

    if(canvas&&(prefersReducedMotion.matches||saveData)){
        // Engine never started; still listen so turning the preference off works without a reload.
    ...
    w.addEventListener('scroll',()=>{
        if(!ticking){
            w.requestAnimationFrame(()=>{
                ...
                if(topBtn){if(scrollTop>400)topBtn.classList.remove('hidden');else topBtn.classList.add('hidden');}
                ticking=false;
            });
            ticking=true;
        }
    ...
    if(topBtn)topBtn.addEventListener('click',()=>w.scrollTo({top:0,behavior:prefersReducedMotion.matches?'auto':'smooth'}));
    ...
    d.querySelectorAll('a[href^="#"]').forEach(anchor=>{
        anchor.addEventListener('click',function(e){
            ...
            w.scrollTo({top:target.getBoundingClientRect().top+w.scrollY-pad,behavior:prefersReducedMotion.matches?'auto':'smooth'});
    ...
    if(contactForm&&submitBtn&&formStatus){
        const setStatus=(msg,state)=>{formStatus.textContent=msg;formStatus.className=`form-status is-${state}`;};
        const errFor=f=>q(f.id+'-err');
        const showError=(f,msg)=>{const p=errFor(f);f.setAttribute('aria-invalid','true');if(p){p.textContent=msg;p.hidden=false;}};
        const clearError=f=>{const p=errFor(f);f.removeAttribute('aria-invalid');if(p){p.hidden=true;p.textContent='';}};

        contactForm.setAttribute('novalidate','');
        ['name','email','form-message'].forEach(id=>{
            const f=q(id);if(f)f.addEventListener('input',()=>{if(f.checkValidity())clearError(f);});
        });
        ...
        contactForm.addEventListener('submit',async(e)=>{
            ...
            submitBtn.disabled=true;submitBtn.textContent='Sending...';setStatus('Sending your message...','pending');
            try{
                ...
                contactForm.reset();
                submitBtn.textContent='Message Sent';submitBtn.classList.add('success');
                setStatus('Message sent successfully.','success');
                setTimeout(()=>{submitBtn.disabled=false;submitBtn.textContent='Send Message';submitBtn.classList.remove('success');},4000);
            }catch(error){
    ...
    // Assemble the address at runtime so it is not sitting in the HTML for scrapers.
    ...
    if(emailHost&&emailHost.dataset.u&&emailHost.dataset.d){
        ...
        a.href=`mailto:${addr}`;a.textContent=addr;a.rel='nofollow';
