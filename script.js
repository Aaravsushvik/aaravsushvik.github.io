...
const state = { lang: "en", paletteOpen: false, palettePreviousFocus: null };

function getPath(object, path) {
 return path.split(".").reduce((value, key) => value && value[key], object);
}
function t(key) {
 return getPath(window.TRANSLATIONS?.[state.lang], key) ??
...
function applyTranslations(lang) {
 ...
 $("#lang-announcer").textContent = lang === "te" ? "భాష తెలుగుకు మార్చబడింది" : lang === "hi" ? "भाषा हिंदी में बदल दी गई है" : "Language changed to English";
 updateThemeUI();
 updateMenuLabel();
}
...
function withViewTransition(fn) {
 // Progressive enhancement: browsers without View Transitions (or with
 // reduced motion requested) just run fn() immediately — no behavior
 // change, just no cross-fade.
...
function initLanguage() {
 ...
 const lang = window.TRANSLATIONS[requested] ? requested : window.TRANSLATIONS[saved] ? saved : "en";
 const selector = $("#lang-select");
 if (selector) {
   selector.value = lang;
   selector.addEventListener("change", (e) => {
     const selected = e.target.value;
     withViewTransition(() => applyTranslations(selected));
...
function initScroll() {
 ...
 const update = () => {
   ...
   for (const section of sections) {
     ...
     if (rect.top <= 140 && rect.bottom > 140) active = section.id;
   }
...
function initReveal() {
 ...
 const nativeScrollTimeline = window.CSS?.supports?.("animation-timeline: view()");
 if (reduced || nativeScrollTimeline || !("IntersectionObserver" in window)) {
...
function initCanvas() {
 ...
 function resize() {
   ...
   const count = Math.min(48, Math.max(18, Math.floor(rect.width / 28)));
 ...
 function draw() {
   ...
   const dark = document.documentElement.classList.contains("dark");
   ctx.fillStyle = dark ? "rgba(41,151,255,.35)" : "rgba(0,113,227,.22)";
   points.forEach((p) => {
     p.x += p.vx;
     p.y += p.vy;
...
function initShare() {
 ...
 btn.addEventListener("click", async () => {
   const data = { title: document.title, text: "Gadiparthi Sai Sushvik — Portfolio", url: window.location.href };
   try {
...
function initEmailLink() {
 // Fix X: no plaintext "mailto:...@..." sits in the static HTML for
 // scrapers to grep — the address is only assembled into a real mailto:
 // link at the moment of a genuine click.
...
async function initForm() {
 ...
 form.addEventListener("submit", async (e) => {
   ...
   let valid = true;
   if (!name.value.trim()) { showError("name-err", "form.name.error"); valid = false; }
   if (!email.value.trim() || !email.validity.valid) { showError("email-err", "form.email.error"); valid = false; }
   if (!message.value.trim()) { showError("form-message-err", "form.message.error"); valid = false; }
   if (!valid) return;
   ...
   try {
     ...
     status.textContent = err.name === "AbortError" ? t("form.failure") : t("form.failure");
   } finally {
...
function initPalette() {
 ...
 function render() {
   ...
   results = commands.filter(([name]) => name.toLowerCase().includes(query));
   list.textContent = "";
   results.forEach(([name, id], index) => {
 ...
 input.addEventListener("keydown", (e) => {
   if (e.key === "ArrowDown") {
     ...
     e.preventDefault();
     if (activeIndex >= 0) {
       const [, id] = results[activeIndex];
       closePalette();
       document.querySelector(id)?.scrollIntoView({ behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" });
     }
   }
   // Escape and Tab are handled natively by <dialog>: Escape fires 'cancel'
   // then 'close' (caught above), and focus is trapped among the dialog's
   // focusable descendants automatically — no manual handling needed.
 });
