document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        if(savedTheme){html.classList.remove('light','dark');html.classList.add(savedTheme);}
    } catch(e) {}
    ...
    if(canvas&&heroSection&&!prefersReducedMotion.matches&&!saveData){
        ...
        class Particle{
            ...
            update(dt){
                ...
                if(this.x<0||this.x>width)this.vx=-this.vx;if(this.y<0||this.y>height)this.vy=-this.vy;
                if(mouse.x!==null){
    ...
    if(menuToggle&&nav){
        menuToggle.addEventListener('click',()=>{
            const isOpen=nav.classList.toggle('open');menuToggle.setAttribute('aria-expanded',isOpen);
            if(isOpen){
                ...
                const firstLink=nav.querySelector('a');if(firstLink)setTimeout(()=>firstLink.focus(),50);
            }else{
                if(supportsInert){if(mainContent)mainContent.inert=false;if(footerElement)footerElement.inert=false;}
                else{if(mainContent)mainContent.removeAttribute('aria-hidden');if(footerElement)footerElement.removeAttribute('aria-hidden');}
            }
    ...
    const initDeferredObservers=()=>{
        if(navLinks.length>0&&sections.length>0){
            const navObserver=new IntersectionObserver((entries)=>{
                entries.forEach(entry=>{if(entry.isIntersecting){const id=entry.target.getAttribute('id');navLinks.forEach(link=>{link.classList.toggle('active',link.getAttribute('href')===`#${id}`);});}});
    ...
    if(contactForm&&submitBtn&&formStatus){
        contactForm.addEventListener('submit',async(e)=>{
            e.preventDefault();submitBtn.disabled=true;submitBtn.textContent='Sending...';formStatus.textContent='Sending your message...';formStatus.style.color='var(--color-text-secondary)';
            try{
                const response=await fetch(contactForm.action,{method:'POST',body:new FormData(contactForm),headers:{'Accept':'application/json'}});
                if(response.ok){contactForm.reset();submitBtn.textContent='Message Sent';submitBtn.classList.add('success');formStatus.textContent='Message sent successfully.';formStatus.style.color='var(--color-success)';}
                else throw new Error('Network response was not ok.');
            }catch(error){
                submitBtn.disabled=false;submitBtn.textContent='Send Message';formStatus.textContent='Something went wrong. Please try again.';formStatus.style.color='var(--color-error)';
            }
        });
    }
});
